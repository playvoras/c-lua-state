...
void RobloxInstance::setCoreScript(bool Boolean)
{
    pDriver->write<bool>(this->self + 0x198, Boolean);
}
...
std::string RobloxInstance::Bytecode()
{
    ...
    if (class_name == xor_a("ModuleScript")) {
        offset = offsets::ModuleScript;
    }
    else if (class_name == xor_a("LocalScript")) {
        offset = offsets::LocalScript;
    }
    else {
        throw std::runtime_error(xor_a("Invalid Script"));
    }

    auto bytecode_ptr = pDriver->read<std::uint64_t>(this->self + offset);
    auto bytecode_ptr_str = pDriver->read<unsigned long long>(bytecode_ptr + 0x10);
    auto bytecode_size = pDriver->read<size_t>(bytecode_ptr + 0x20);


    byte* buffer = new byte[bytecode_size];

    pDriver->read_memory((uintptr_t)bytecode_ptr_str, (uintptr_t)buffer, bytecode_size);


    std::cout << xor_a("[~] Bytecode size: ") << bytecode_size << std::endl; */
